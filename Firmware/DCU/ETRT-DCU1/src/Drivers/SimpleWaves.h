//Simple waveform Library
#ifndef SIMPLEWAVES_H
#define SIMPLEWAVES_H

#include "Arduino.h"

#define SIN_TABLE_LENGTH 1024
#define SIN_TABLE_ZERO 0x800
...
public:
	...
	void begin(waveType_t wave = SINE);
	void update();
	void setType(waveType_t wave);
	void set(float frequency, float amplitude);
	void set(float frequency, float amplitude, waveType_t wave);
  void pause() {paused = true;}
  void resume() {paused = false;}
	float output() {return waveOut*amp;}
  uint16_t  getScaledAudio(uint16_t maxVal);
	void setFrequency(float fHertz) {freq = fHertz;}
	void setAmplitude(float amplitude) {amp = amplitude;}
	float getDeltaT() {return deltaT;}
	
	unsigned long debugGet0(){return oneShotChange;}
  unsigned long debugGet1(){return oneShotEnd;}
  unsigned long debugGet2(){return 0;}
  float debugGetF0(){return freq;}
private:
void updateTimeDelta();
  ...
  void updateOneshot();
  void updateHalfshot();
	float getSin(uint16_t index);
	void updateSquarePeriod();
	...
	float sinStep = 0.0;
	float triangleDirection = 1.0;
	unsigned long nextSquareUpdate = 0;
 unsigned long oneShotEnd = 0;
 unsigned long oneShotChange = 0;
 uint8_t oneShotState = 0;
	float freq = 100.0;
	float amp = 1.0;
	float squareState = 1.0;
	uint16_t sinIndex = 0;
	bool paused = false;
  
	uint16_t sinTable[SIN_TABLE_LENGTH] = 
